...
const CleanerExperienceScreen: React.FC<CleanerExperienceProps> = ({
  navigation,
}) => {
  ...
  const validateExperience = (): boolean => {
    if (
      isNaN(+cleaningExperience) ||
      +cleaningExperience < 0 ||
      +cleaningExperience > 50
    ) {
      setErrors((prev) => ({
        ...prev,
        cleaningExperience: "Experience must be a number between 0 and 50.",
      }));
      return false;
    }
    return true;
  ...
  const validateSpecialty = (): boolean => {
    if (cleaningSpecialty.length === 0) {
      setErrors((prev) => ({
        ...prev,
        cleaningSpecialty: "Please select at least one specialty.",
      }));
      return false;
    }
    return true;
  ...
  const validateDays = (): boolean => {
    if (daysAvailable.length === 0) {
      setErrors((prev) => ({
        ...prev,
        daysAvailable: "Please select at least one day.",
      }));
      return false;
    }
    return true;
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <SafeAreaView>
        ...
        <View style={styles.form}>
          ...
          />
          {!!errors.cleaningExperience && (
            <Text style={styles.errorText}>{errors.cleaningExperience}</Text>
          )}
            ...
            {specialties.map((spec) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  cleaningSpecialty.includes(spec) && styles.selectedItem,
                ]}
                onPress={() =>
                  setCleaningSpecialty((prev) => toggleSelection(prev, spec))
                }
              >
                <Text>{spec}</Text>
              </TouchableOpacity>
          ...
          {!!errors.cleaningSpecialty && (
            <Text style={styles.errorText}>{errors.cleaningSpecialty}</Text>
          )}
          ...
          <View style={styles.multiSelectContainer}>
            {days.map((day) => (
              <TouchableOpacity
                ...
                style={[
                  styles.multiSelectItem,
                  daysAvailable.includes(day) && styles.selectedItem,
                ]}
                onPress={() =>
                  setDaysAvailable((prev) => toggleSelection(prev, day))
                }
              >
                <Text>{day}</Text>
              </TouchableOpacity>
            ...
            {!!errors.daysAvailable && (
              <Text style={styles.errorText}>{errors.daysAvailable}</Text>
            )}
          </View>
